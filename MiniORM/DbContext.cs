...
namespace MiniORM
{
    public abstract class DbContext
    {
        ...
        protected DbContext(string connectionString)
        {
            ...
            this.MapAllRellations();
        }

        public void SaveChancges()
        {
            var dbSets = this.dbSetProperties.Select(pi => pi.Value.GetValue(this)).ToArray();

            foreach (IEnumerable<object> dbSet in dbSets)
            {
                var invalidEntities = dbSet.Select(entity => !IsObjectValid(entity)).ToArray();
            ...
            using (new ConnectionManager(connection))
            {
                ...
                {
                    foreach (Enumerable dbSet in dbSets)
                    {
    ...
    where TEntity : class, new()
        ...
        private void MapAllRelations()
        {
            foreach (var dbSetProperty in this.dbSetProperties)
            {
                ...
                var mapRelationsGeneric = typeof(DbContext).GetMethod("MapRelations", BindingFlags.Instance | BindingFlags.NonPublic).MakeGenericMethod(dbStType);
            ...
            where TEntity: class, new()
        {
            ...
            var collections = entityType.GetProperties().
                Where(pi => pi.PropertyType.IsGenericType && pi.PropertyType.GetGenericTypeDefinition() == typeof(ICollection<>)).ToArray();
            ...
            where TEntity: class
        {
            ...
            var columns = GetEntitiesColumnNames(table);
        ...
        {
            var dbSets = this.GetType().GetProperties().Where(pi => pi.PropertyType.GetGenericTypeDefinition() == typeof(DbSet<>))
